...
const ArticleCard: React.FC<{ article: Article }> = ({ article }) => (
    <Link to={`/article/${article.id}`} className="block group overflow-hidden rounded-lg shadow-lg bg-white h-full flex flex-col transform hover:-translate-y-2 transition-transform duration-300 border border-gray-200 hover:shadow-xl hover:border-brand-gold">
...
const ArticleListPage: React.FC = () => {
    ...
    const filteredArticles = useMemo(() => {
        if (!activeCategory) {
            return ARTICLES_DATA; // Show all if no category or category is invalid
        }
        return ARTICLES_DATA.filter(article => article.category === activeCategory);
    }, [activeCategory]);

    const getNavLinkClass = (isActive: boolean) => 
        `px-4 py-2 rounded-full text-sm font-semibold transition-colors duration-300 ${isActive ? 'bg-brand-gold text-white' : 'bg-white text-gray-700 border border-gray-300 hover:bg-brand-gold hover:text-white'}`;

    return (
        <div className="bg-brand-bg-alt py-20 pt-32 min-h-screen">
